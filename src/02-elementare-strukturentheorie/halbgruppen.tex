...
\begin{example}\label{ex:selbstabbildungsmonoid}
    Für eine beliebige Menge $M$ ist die Menge aller Funktionen von $M$ nach $M$ mit der Verkettung und $\id_M$ als neutrales Element ein Monoid $\mathfrak{M} = (M^M, \circ, \id_M)$.
\end{example}
...
\begin{lemma}
    Neutrale und inverse Elemente in Monoiden sind eindeutig.
\end{lemma}
...
\begin{remark}
    ...
    Ist der Generator (die Variablenmenge) $X$ unendlich, so ist das erzeugte Monoid \emph{total frei}\index{Monoid!total frei} über $X$, das heißt es gelten genau die Gesetze, die in der Varietät gelten. Da die Aritäten stets endlich sind, kann jedes Gesetz über einer Sprache mit Hilfe einer beliebigen unendlichen Variablenmenge ausgedrückt werden. Daher folgt dies aus folgender Bemerkung.
\end{remark}
...
Wichtig für den folgenden Satz ist die Menge der Primzahlen $\mathbb{P}$.
...
\begin{definition}
    Sei $\mathfrak{M} = (M,\cdot,e)$ ein Monoid und $a \in M$. Gilt für alle $b, b' \in M$
    \begin{itemize}
...
\begin{remark}
    Es stellt sich die Frage ob es möglich ist ein Monoid $(M, \cdot, e)$ in eine Gruppe einzubetten. Wir beobachten, dass in einer Gruppe alle Elemente sowohl links-, als auch rechtskürzbar sind. Notwendig für Einbettbarkeit von einem Monoid $\mathfrak{M} = (M, \cdot, e)$ in eine Gruppe ist also jedenfalls, dass alle $a \in M$ kürzbar sind. Das Monoid aller Selbstabbildungen einer Menge aus \cref{ex:selbstabbildungsmonoid} hat nichtkürzbare Elemente, ist also nicht in eine Gruppe einbettbar.

    Hinreichend hingegen ist die obige Kürzbarkeit mit der zusätzlichen Forderung das $\mathfrak{M}$ kommutativ ist (siehe Satz \ref{theorem:monoid-gruppe-einbettung}).
\end{remark}

\begin{example} {\ }
    \begin{enumerate}
        \item Betrachte die Gruppe $(\Gl_2(\mathbb{R}), \cdot\footnote{Hier bezeichnet $\cdot$ die Matrixmultiplikation.}, I_2)$ und das (nicht kommutative) Untermonoid $\mathfrak{H} := (\Gl_2(\mathbb{R}) \cap \mathbb{Z}^{2 \times 2}, \cdot, I_2)$.
...
\begin{theorem}\label{theorem:monoid-gruppe-einbettung}
    Sei $\mathfrak{M} = (M, \cdot, e)$ ein kommutatives Monoid und jedes $a \in M$ kürzbar\footnote{Aufgrund der Kommutativität reicht es sogar lediglich Links- oder Rechtskürzbarkeit zu fordern.}. Dann gilt
    \begin{enumerate}
        \item $\sim\; \subseteq (M^2)^2$ mit
        $$ (a,b) \sim (c,d) :\Leftrightarrow a \cdot d = b \cdot c $$
        ist eine Kongruenzrelation auf $\mathfrak{M}^2$.
        \item $\mathfrak{M}^2 /_\sim$ ist eine Gruppe.
        \item Die Abbildung
        $$ \varphi :\; \mathfrak{M} \to \mathfrak{M}^2 /_\sim, \;a \mapsto [(a,e)]_\sim $$
        ist eine \emph{Einbettung}\index{Einbettung}, also ein injektiver Homomorphismus.
        \item Sei $\mathfrak{G}$ eine Gruppe, so gibt es für alle injektiven Homomorphismen $\psi : \mathfrak{M} \to \mathfrak{G}$ einen injektiven Homomorphismus $\overline{\psi} : \mathfrak{M}^2 /_\sim \to \mathfrak{G}$ mit $\overline{\psi} \circ \varphi = \psi$.
    \end{enumerate}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \node at (3, 0) {$\mathfrak{M}$};
            \node at (4.6, 1) {$\mathfrak{M}^2 /_\sim$};
            ...
            \node at (0.5, -2.6) {$\mathfrak{M}$};
            \node at (0.5, -4.2) {$\mathfrak{M}^2 /_\sim$};
        ...
        \end{tikzpicture}
        \caption{Visualisierung der Einbettung von $\mathfrak{M}$ in die Gruppen $\mathfrak{G}, \mathfrak{M}^2 /_\sim$}
    \end{figure}
\end{theorem}
...
\begin{proof} {\ }
    \begin{enumerate}
        ...
        \item Hier liefert \cref{theorem:birkhoff} (Birkhoff), dass $\mathfrak{M}^2 /_\sim$ ein Monoid mit neutralem Element $[(e,e)]_\sim$ ist. Wir bemerken, dass $(a, b) \sim (e, e) \Leftrightarrow ae = be \Leftrightarrow a = b$, also ist $[(e,e)]_\sim = \{ (a, a) \mid a \in M \}$ unser neutrales Element in $\mathfrak{M}^2 /_\sim$.
        
        Wegen
        $$ [(a,b)]_\sim \cdot [(b,a)]_\sim = [(ab, ab)]_\sim = [(e,e)]_\sim $$
        ist $[(b,a)]_\sim$ invers zu $[(a,b)]_\sim$, womit $\mathfrak{M}^2 /_\sim$ mit obiger Inversenbildung eine Gruppe ist.

        \item Es gilt
        $$ \varphi(e) = [(e,e)]_\sim \quad \textrm{neutral in } \mathfrak{M}^2 /_\sim, $$
        sowie für $a, b \in M$
        $$ \varphi(ab) = [(ab, e)]_\sim = [(a,e)]_\sim \cdot [(b,e)]_\sim = \varphi(a) \cdot \varphi(b), $$
        womit $\varphi$ ein Homomorphismus ist.

        Seien nun $a,b \in M$ mit $\varphi(a) = \varphi(b)$, also $[(a,e)]_\sim = [(b,e)]_\sim$, so folgt $a = ae = eb = b$, womit $\varphi$ injektiv ist.

        \item Sei \obda $\psi = \id_M$\footnote{Diese Einschränkung ist möglich, da $\psi$ ein injektiver Homomorphismus ist.} und definiere $ \overline{\psi} :\; \mathfrak{M}^2 /_\sim \to \mathfrak{G}, \;[(a,b)]_\sim \mapsto a \cdot b^{-1} $.
        
        Seien $a,b,c,d \in M$ beliebig mit $a b^{-1} = c d^{-1}$, so folgt $ad = bc$, also $[(a,b)]_\sim = [(c,d)]_\sim$, womit $\overline{\psi}$ injektiv ist.
        ...
        Zuletzt betrachten wir $$ (\overline{\psi} \circ \varphi)(a) = \overline{\psi}(\varphi(a)) = \overline{\psi}([(a,e)]_\sim) = a \cdot e^{-1} = a.$$ Daher ist $\overline{\psi} \circ \varphi = \psi$.
    \end{enumerate}
\end{proof}