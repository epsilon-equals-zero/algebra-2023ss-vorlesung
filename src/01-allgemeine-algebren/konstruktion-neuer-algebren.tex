...
\begin{remark}
    Sei $\mathfrak{V}=(V,+,0,-,(m_k)_{k\in\mathfrak{K}})$ ein Vektorraum mit $\vert V\vert > 1$ über einem Körper $\mathfrak{K}$.
...
\begin{proof}
    ...
    erhält man $\tilde{t}^\mathfrak{A}(s_1,\ldots,s_n)=t^\mathfrak{A}(a_1,\ldots,a_\ell)=a\in\{t^\mathfrak{A}(s_1,\ldots,s_n)\mid t(x_1,\ldots,x_n)\in T(X)\}.$\footnote{Genau genommen ist die Konstruktion von $\tilde{t}$ nicht notwendig, da schon $t \in T(X)$ ist. Wenn man allerdings fordert, dass nur bei $\var(t)=\{x_1,\ldots,x_n\}$ auch $t(x_1,\ldots,x_n)$ geschrieben wird, dann wird der zusätzliche Schritt benötigt.}
\end{proof}
