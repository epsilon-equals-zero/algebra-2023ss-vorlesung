...
\begin{definition}
    ...
    Weiter werden die \emph{Variablen eines Terms}\index{Term!Variablen} rekursiv definiert. Für $x \in X$ ist $\var(x) := \{x\}$ und für $t = f_i(t_1, \ldots, t_{n_i})$ ist $\var(t) := \bigcup_{j \in \{1, \ldots, n_i\}} \var(t_j)$. Für einen Term $t$ werden wir auch $t(\var(t))$ beziehungsweise im Fall $\var(t)\subseteq\{x_1,\ldots,x_n\}$ auch $t(x_1,\ldots,x_n)$ schreiben.
\end{definition}
